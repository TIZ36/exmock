defmodule Ezx.Orm.Mapper do
  ...
  defmacro mapper(mapper_name, [validator: func, params: po_type], do: block) when is_binary(mapper_name) do
  ...
  defmacro mapper(mapper_name, [params: po_type], do: block) when is_binary(mapper_name) do
    quote do
      mapper(unquote(mapper_name), [validator: &__MODULE__.no_validate/1, params: unquote(po_type)], do: unquote(block))
    end
  end
end